...
@click.command()
@click.option('--url', '-u', 
              help='URL direta do stream m3u8')
@click.option('--encoded', '-e', 
              help='URL do stream codificada em base64')
@click.option('--output', '-o', 
              default='video.mp4',
              help='Nome do arquivo de saída (padrão: video.mp4)')
@click.option('--referer', '-r', 
              help='URL da página de referência (padrão: https://www.weekseries.info/)')
@click.option('--no-convert', 
              is_flag=True,
              help='Não converter para MP4, manter apenas .ts')
...
def main(url, encoded, output, referer, no_convert):
    ...
    if no_convert:
        if not output_file.endswith('.ts'):
            output_file = (output_file.replace('.mp4', '.ts') 
